...
#if defined HAVE_DECL_SHMEM_INT_ATOMIC_XOR && HAVE_DECL_SHMEM_INT_ATOMIC_XOR
...
#define FUNC_VALUE  shmem_int_atomic_xor
#ifdef QUICK_TEST
 ...
 ***************************************************************************/
int osh_atomic_tc23(const TE_NODE *node, int argc, const char *argv[])
{
#if TEST_ENABLED
    int rc = TC_PASS;
...
static int test_item1(void)
{
    ...
    if (shmem_addr)
    {
        ...
        *shmem_addr = 0;
        ...
        {
            FUNC_VALUE(shmem_addr, i + my_proc, root_proc);
        }
        shmem_barrier_all();
...
static int test_item2(void)
{
    ...
    if (shmem_addr)
    {
        ...
        *shmem_addr = 0;
        ...
        /* Define expected value */
        for (i = 0; i < __cycle_count; i++) {
            expect_value ^= i + my_proc + 1;
        }

        shmem_barrier_all();
        for (i = 0; (i < __cycle_count) && (rc == TC_PASS); i++)
        {
            FUNC_VALUE(shmem_addr, i + peer_proc + 1, peer_proc);
        }
        shmem_barrier_all();

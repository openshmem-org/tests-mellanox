...
#if defined HAVE_DECL_SHMEM_LONGLONG_ATOMIC_XOR && HAVE_DECL_SHMEM_LONGLONG_ATOMIC_XOR
...
#define FUNC_VALUE  shmem_longlong_atomic_xor
 ...
 ***************************************************************************/
int osh_atomic_tc25(const TE_NODE *node, int argc, const char *argv[])
{
#if TEST_ENABLED
    int rc = TC_PASS;
...
static int test_item2(void)
{
    ...
    if (shmem_addr)
    {
        ...
        *shmem_addr = 0;
        ...
        /* Define expected value */
        for (i = 0; i < __cycle_count; i++) {
            expect_value ^= PROC_VALUE(i + my_proc + 1);
        }
